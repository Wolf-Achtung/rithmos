...
/** The patterns as the player meets them before the name (CLAUDE.md 2: the feedback teaches the rule). */
export const patternName: Record<HarmonyKind, string> = {
  arithmetic: 'gleiche Schritte',
  geometric: 'gleiche Faktoren',
  musical: 'Schritte wie die Außenzahlen',
};
export const patternShort: Record<HarmonyKind, string> = {
  arithmetic: 'Schritte',
  geometric: 'Faktoren',
  musical: 'wie außen',
};
...
export const texts = {
  ...
  chainLink: (kind: HarmonyKind) => patternShort[kind],
  ...
  practiceLevelName: ['gleiche Schritte', 'gleiche Faktoren', 'wie die Außenzahlen', 'welches Muster?', 'vier Zahlen'] as const,
  ...
  whichQuestion: 'Welches Muster ist das?',
  whichOffer: (kind: HarmonyKind, example: readonly number[]) => `${patternName[kind]} · ${example.join(' · ')}`,
  whichWrong: (kind: HarmonyKind) => `Nicht „${patternName[kind]}“. Noch ein Versuch.`,
  whichRevealed: (kind: HarmonyKind) => `Es war „${patternName[kind]}“, ${kindName[kind]}.`,
  ...
  quotaLine: (hunt: number, rules: number) => `Heute noch: ${hunt === 1 ? 'eine Jagd' : `${hunt} Jagden`} · ${rules === 1 ? 'eine Regelfrage' : `${rules} Regelfragen`}.`,
