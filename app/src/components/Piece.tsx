...
export function Piece({ piece, size }: Props) {
  ...
  if (piece.shape === 'round') {
    return <View style={[styles.base, { width: d, height: d, borderRadius: d / 2, backgroundColor: fill }]}>{label}</View>;
  }
  ...
  if (piece.shape === 'square') {
    return <View style={[styles.base, { width: d, height: d, borderRadius: 2, backgroundColor: fill }]}>{label}</View>;
  }
  // pyramid: a square with a smaller stacked square, marked by a double border
  return (
    <View style={[styles.base, { width: d, height: d, borderRadius: 2, backgroundColor: fill, borderWidth: 2, borderColor: ink }]}>
